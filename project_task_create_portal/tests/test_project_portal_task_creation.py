...
class TestProjectPortalTaskCreation(TestProjectPortalCommon, HttpCaseWithUserPortal):
    ...
    def test_project_portal_task_editing_allowed(self):
        ...
        # Project without portal stage should not allow editing
        self.project.portal_stage_id = False
    ...
    def test_creation_and_edit_rights(self):
        ...
        self.project.write({"portal_user_ids": [(4, self.user_portal.id)]})
    ...
    def test_default_task_name(self):
        # If we allow editing the 'name' field, default includes the portal user’s name
        name_field = self.env["ir.model.fields"].search(
            [("model", "=", "project.task"), ("name", "=", "name")]
        )
        self.project.write({"portal_create_task_fields": [(4, name_field.id)]})
        default_name = self.project.with_user(self.user_portal.id).get_default_values(
            "name", self.project
        )
        self.assertIn("Portal task from", default_name)
